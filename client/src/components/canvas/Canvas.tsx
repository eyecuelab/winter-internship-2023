...
function Canvas(props: Props) {
  const {
    ...
    roomGameRef,
    myGameRef,
    setRoomGameStateWrapper,
    setMyGameStateWrapper,
    lastKeyRef,
  } = props;
  ...
  const updateKartYMovements = () => {
    if (isWaitingForGameModalOpen === false) {
      ...
      if (myColor) {
        const kart: Kart =
          roomGameRef.current?.karts.get(myColor) ?? new Kart(); //not sure about this..
  ...
  const updateKartXMovements = () => {
    if (isWaitingForGameModalOpen === false) {
      ...
      if (myColor) {
        const kart: Kart =
          roomGameRef.current?.karts.get(myColor) ?? new Kart();
        const velocityUnit = kart.isGhost ? 20 : 10;
    ...
    useEffect(() => {
      ...
      pelletImg.src = `data:image/svg+xml;base64,${window.btoa(
        pelletSvgString
      )}`;
        ...
        if (myGameRef.current) {
          ...
          if (socketId === socketIds[numberOfUsers - 1]) {
            if (numberOfUsers % 2 === 0) {
              const spawnPosition =
                spawnPointsRef.current[numberOfUsers / 2 - 1];
